...
async def add_model(request, picture_cover, model_object, username, db):
    ...
    check_db= db.query(models.ModelObject).filter(models.ModelObject.id == request.id.upper())
    if check_db.first():
        raise HTTPException(status_code= status.HTTP_409_CONFLICT, detail= f"Model with this name: '{request.id}' already exist.")
    ...
    upload_picture_cover= await storage.upload_model_picture(picture_cover, request.id)

    upload_model= None
    if os.path.splitext(model_object.filename)[1] != ".zip":
        upload_model= await storage.upload_model_file_only(model_object, request.id)
    else:
        with zipfile.ZipFile(io.BytesIO(model_object.file.read()), 'r') as zip_ref:
            ...
            upload_model= await storage.upload_model_zip(request.id, model_folder_path)
